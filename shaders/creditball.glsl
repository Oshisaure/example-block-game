...
vec4 effect(vec4 color, Image image, vec2 uvs, vec2 screen_coords){
    ...
    vec3 moonpos  = vec3(-300., -250.+10.*cos(time*.2), 1000.);
    ...
    if (moonpoint.valid) {
        ...
        newpixel.rgb *= dot(moonpoint.normaltransform * mapnormal, LIGHTDIR);
        newpixel.a = 1;
        
    } else {
        // floating amigaball
        vec3 amigapos  = vec3(100., 65.+10.*cos(time*.2), 400.);
        vec3 amigaaxis = normalize(vec3(5.+cos(time*.04), -10.+sin(time*.04), -5.));
        vec3 amigaback = normalize(cross(amigaaxis, vec3(0.,0.,1.)));
        vec3 amigaleft = cross(amigaaxis, amigaback);
        
        ballpoint amigapoint = pointOnBall(uvdir, amigapos, 65., amigaaxis, amigaback, amigaleft);
        if (amigapoint.valid) {
            amigapoint.latitude = mod(amigapoint.latitude - time*.25, TAU);
            // amiga ball
            ...
            newpixel.rgb = vec3(1.,red,red) * (dot(amigapoint.normal, -uvdir)*.7+.3);
			newpixel.a = 1;
        }
    }
    return newpixel;
