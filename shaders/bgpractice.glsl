...
const vec3 LIGHTDIR = normalize(vec3( -40.,  -20.,  -50.));
const vec3 moonpos  = vec3(0., 52., 10.);
const vec3 moonaxis = vec3(1., 0., 0.);
const vec3 moonback = vec3(0., 0., 1.);
const vec3 moonleft = vec3(0., 1., 0.);
#define AMIGAROWS 6.
#define AMIGACOLS 12.
...
vec4 effect(vec4 color, Image image, vec2 uvs, vec2 screen_coords){
    ...
    ballpoint moonpoint = pointOnBall(uvdir, moonpos, 50., moonaxis, moonback, moonleft);
    if (moonpoint.valid) {
        // test
        // newpixel.xyz = vec3(longitude, latitude, 0.);
        // moon texture
        moonpoint.latitude = mod(moonpoint.latitude - .01*time, TAU);
        newpixel = Texel(moontex, vec2(moonpoint.latitude/TAU, moonpoint.longitude/PI +.5))*2;
        ...
        // floating amigaball
        vec3 amigapos  = vec3(150., -70.+10.*cos(time*.2), 400.);
        vec3 amigaaxis = normalize(vec3(.3, cos(time*.04), sin(time*.04)));
        vec3 amigaback = normalize(cross(amigaaxis, vec3(0.,0.,1.)));
        vec3 amigaleft = cross(amigaaxis, amigaback);
        
        ballpoint amigapoint = pointOnBall(uvdir, amigapos, 100., amigaaxis, amigaback, amigaleft);
        if (amigapoint.valid) {
            amigapoint.latitude = mod(amigapoint.latitude + time*.1, TAU);
            // amiga ball
            ...
            newpixel.xyz = vec3(1.,red,red) * dot(amigapoint.normal, LIGHTDIR);
        } else {
